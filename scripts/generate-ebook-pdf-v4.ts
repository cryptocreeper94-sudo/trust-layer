...
const concordanceLocations: Map<string, ConcordanceLocation> = new Map();
...
function renderElement(
  ...
  switch (element.type) {
    ...
    case 'h3':
      ...
      if (inConcordance) {
        concordanceLocations.set(h3Normalized, {
...
async function generatePDF() {
  ...
  console.log('Creating PDF with footnote markers...');
  ...
  return new Promise<void>((resolve, reject) => {
    writeStream.on('finish', () => {
      ...
      console.log(`Footnote references: ${footnoteCounter}`);
      console.log(`Concordance entries mapped: ${concordanceLocations.size}`);
      
      console.log('\nNote: PDF internal hyperlinks require a two-pass system.');
      console.log('Blue [#] markers indicate concordance references.');
      console.log('Concordance entries show which footnotes reference them.');
