...
export default function GuardianScannerRegistry() {
  usePageAnalytics();
  const [searchQuery, setSearchQuery] = useState('');
  const [filterType, setFilterType] = useState<string>('all');

  const { data: agents, isLoading } = useQuery({
    queryKey: ['/api/guardian-scanner/registry'],
    queryFn: async () => {
      const res = await fetch('/api/guardian-scanner/registry');
      if (!res.ok) return [];
      return res.json();
    },
  });
        ...
        >
          <GlassCard className="p-8 bg-gradient-to-br from-slate-900/50 to-purple-900/20">
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                <h3 className="text-xl font-bold text-white mb-4">What is Guardian Scanner Certification?</h3>
