...
- **Authentication**: Firebase Auth only (Replit Auth removed Jan 2026). Supports email/password, Google, GitHub, Apple, and email+PIN quick login. Server-side Firebase token verification using Firebase Admin SDK with revocation checking (requires FIREBASE_SERVICE_ACCOUNT secret). WebAuthn/Passkeys available. PIN authentication (4-6 digits) for returning users - stored as SHA256 hash with user ID salt, uses timing-safe comparison. PIN login is convenience only; full API access requires Firebase session.
