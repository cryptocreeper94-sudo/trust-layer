...
- **Trust Book**: Premium ebook publishing and reading platform at `/trust-book`. Features 5-tab interface (Discover, Browse, My Library, Write, Publish). Flagship title: "Through The Veil" (107K words, 52 chapters, 13 parts, $4.99 with 4-chapter free preview). Category system: Fiction (12 subcategories) and Non-Fiction (13 subcategories) with filterable browsing. Personal library with reading progress tracking. AI Book Author Agent (GPT-4o powered writing assistant with persistent sessions). Author publishing portal with category/subcategory selection, 70% royalty, admin review workflow. Supports AI narration, multi-format reading (online/PDF/EPUB), blockchain-verified provenance. **Author Payout System**: Stripe Connect Express accounts for direct bank payouts. Authors connect bank via onboarding flow, earnings tracked per sale (70% author / 30% platform), 7-day settlement period, automated transfers via `stripe.transfers.create`. Concurrency-safe with status locking (pending→processing→paid) and rollback on failure. Author Earnings Dashboard shows total sales, royalties, pending/paid amounts, and payout button. DB tables: `ebook_purchases`, `published_books`, `user_library`, `ai_writing_sessions`, `author_profiles`, `author_earnings`. Key files: `server/author-payouts.ts`, API routes at `/api/ebook/author/*`. Listed in ecosystem as App #31 at `https://trustbook.tlid.io`.
- **Launch Countdown**: Master launch roadmap at `/launch` with August 23, 2026 target date (1-year anniversary & founder's 50th birthday). Real-time countdown timer, 6-phase checklist (Core Infrastructure, DeFi Ecosystem, Cross-Chain Bridge, Security & Guardian, Ecosystem & Apps, Launch Preparation) with done/in-progress/pending status tracking. Featured prominently in hamburger menu and presale page countdown banner. "One Year. One Vision. Launch Day." branding.
- **Unified Transaction Tracking**: Every Stripe purchase (presale, subscription, credits, Guardian, ebook, crowdfund) is logged to `user_transactions` table with a SHA-256 blockchain tx hash, linked to user ID. Hub displays real transaction history at `/my-hub`. API: `GET /api/user/transactions`. Backfilled existing presale purchases.
- **My Hub / User Portal**: Personalized greeting (time-of-day in Central time + first name), member number, explorer address (0x + SHA-256 of user ID), Signal Allocation from presale_purchases, shell balance, transaction history with blockchain hashes. Nav shows "Trust Layer | Your Portal".
