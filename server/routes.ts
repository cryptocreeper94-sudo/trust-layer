...
export async function registerRoutes(
  ...
  // Get member number based on signup order
  app.get("/api/user/member-number", isAuthenticated, async (req: any, res) => {
    try {
...
const { trustLayerId } = await response.json();`
  ...
  app.get("/api/wallet/external", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: "Authentication required" });
  ...
  app.post("/api/wallet/external", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: "Authentication required" });
  ...
  app.delete("/api/wallet/external/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id;
...
Keep responses concise (2-3 sentences max), friendly, and helpful. If asked about prices, balances, or specific data, explain that you can guide them to the relevant page. Use casual, warm language like you're a helpful friend. Do not use emojis or describe emojis in text form (like ":)" or "smile emoji"). Just communicate naturally without any emoji references.`;
  ...
  // Register Chronicles Daily Life System (Career, Needs, Session Context)
  const { registerDailyLifeRoutes } = await import("./chronicles-daily-life-routes");
  registerDailyLifeRoutes(app);
...
// Estate persistence endpoints
  ...
  const GARAGEBOT_AFFILIATE_ID_REGEX = /^GB-[ABCDEFGHJKLMNPQRSTUVWXYZ2-9]{6}$/;

  function isValidEcosystemAffiliateId(id: string): boolean {
    return ECOSYSTEM_AFFILIATE_ID_REGEX.test(id) || GARAGEBOT_AFFILIATE_ID_REGEX.test(id);
  }

  const ECOSYSTEM_APP_URLS: Record<string, string> = {
  ...
  const HANDOFF_SPEC = {
    ...
    purpose: "Complete integration specification for Trust Layer (dwtl.io) to recognize, track, and sync affiliate activity across all ecosystem platforms.",
    overview: {
      summary: "Trust Layer unifies affiliate tracking across the ecosystem. Each platform runs its own referral program with its own reward structure. Trust Layer affiliate IDs (tl- prefix) are recognized across all platforms, and all platform-native affiliate codes (tldc-, he-, GB-) are recognized by Trust Layer for cross-platform dashboard views.",
      platforms: [
        { name: "TL Driver Connect", domain: "tldc", prefix: "tldc-", description: "Nationwide driver services platform — food delivery, parts, concierge, expense tracking, entertainment" },
        { name: "Happy Eats", domain: "he", prefix: "he-", description: "Food truck ordering platform — zone-based batch ordering in Nashville I-24 Corridor with 11 AM cutoff" },
        { name: "Trust Layer", domain: "tl", prefix: "tl-", description: "Parent ecosystem at dwtl.io — affiliate hub, staffing, financial services" },
        { name: "GarageBot", domain: "garagebot", prefix: "GB-", description: "Smart automation for vehicle maintenance and garage management. Inbound affiliate program with purchase commissions and Pro subscription bonuses.", handoff_endpoint: "/api/affiliate-program/trustlayer/:code" }
      ],
      business_models: {
        "tldc_he": "Happy Eats charges a 15% platform fee on all vendor orders. Affiliate revenue share comes out of this 15% fee.",
        "garagebot": "GarageBot pays 10% of its affiliate commission on referred user purchases, plus $5 one-time Pro conversion bonus and $2/month recurring per active Pro referral. $100 purchase qualification threshold."
      }
    },
    affiliate_id_system: {
      formats: [
        { prefix: "tldc-", pattern: "{prefix}{base36-timestamp}-{8-char-hex-random}", platform: "TL Driver Connect" },
        { prefix: "he-", pattern: "{prefix}{base36-timestamp}-{8-char-hex-random}", platform: "Happy Eats" },
        { prefix: "tl-", pattern: "{prefix}{base36-timestamp}-{8-char-hex-random}", platform: "Trust Layer" },
        { prefix: "GB-", pattern: "GB-{6-char-alphanumeric}", charset: "ABCDEFGHJKLMNPQRSTUVWXYZ23456789", platform: "GarageBot" }
      ],
      validation_regex: {
        ecosystem_standard: "^(tldc|he|tl)-[a-z0-9]+-[a-f0-9]{8}$",
        garagebot: "^GB-[ABCDEFGHJKLMNPQRSTUVWXYZ2-9]{6}$",
        any_platform: "Matches either ecosystem_standard OR garagebot pattern"
      },
      cross_platform_recognition: "All prefixed IDs are recognized across the ecosystem. Trust Layer can query any platform's handoff endpoint to retrieve affiliate stats for cross-platform dashboard views."
    },
    reward_structures: {
      tldc_he: {
        ...
        ongoing_revenue_share: {
          tiers: [
            { tier_name: "Starter", qualified_referrals: "1-9", percentage: 1 },
            { tier_name: "Builder", qualified_referrals: "10+", percentage: 2 }
          ]
        },
        activation: { min_days: 14, min_revenue_cents: 10000 }
      },
      garagebot: {
        purchase_commission: "10% of GarageBot affiliate commission on referred user purchases",
        pro_conversion_bonus: { amount: "$5", description: "One-time bonus when referred user upgrades to Pro" },
        pro_recurring: { amount: "$2/month", description: "Per active Pro referral, recurring monthly" },
        qualification_threshold: "$100 total referred purchases",
        minimum_payout: "$20"
      }
    },
    payout_system: {
      ...
      note: "Payouts are managed per-platform. Trust Layer can display balances but payouts are disbursed by the platform where earnings were generated."
    },
    api_endpoints: {
      ...
      cross_platform_lookup: { method: "GET", url: "/api/affiliates/lookup/:code", auth: "None", description: "Looks up any ecosystem affiliate code (tl-, tldc-, he-, GB-) and returns stats from the originating platform's handoff endpoint" },
      create_referral: { method: "POST", url: "/api/affiliates/referrals", auth: "HMAC (x-api-key, x-api-signature, x-api-timestamp)" },
      activate_referral: { method: "POST", url: "/api/affiliates/referrals/:id/activate", auth: "HMAC (x-api-key, x-api-signature, x-api-timestamp)" },
  ...
  app.get("/api/affiliates/format-spec", async (_req: Request, res: Response) => {
    res.json({
      affiliate_id: {
        formats: [
          { prefix: "tldc-", pattern: "{prefix}{base36-timestamp}-{8-char-hex-random}", platform: "TL Driver Connect", example: "tldc-m1abc2d-3e4f5a6b" },
          { prefix: "he-", pattern: "{prefix}{base36-timestamp}-{8-char-hex-random}", platform: "Happy Eats", example: "he-m1abc2d-3e4f5a6b" },
          { prefix: "tl-", pattern: "{prefix}{base36-timestamp}-{8-char-hex-random}", platform: "Trust Layer", example: "tl-m1abc2d-3e4f5a6b" },
          { prefix: "GB-", pattern: "GB-{6-char-alphanumeric}", charset: "ABCDEFGHJKLMNPQRSTUVWXYZ23456789", platform: "GarageBot", example: "GB-K7HN3R" }
        ],
        validation_regex: {
          ecosystem_standard: "^(tldc|he|tl)-[a-z0-9]+-[a-f0-9]{8}$",
          garagebot: "^GB-[ABCDEFGHJKLMNPQRSTUVWXYZ2-9]{6}$"
      ...
      reward_structures: {
        tldc_he: {
          ...
          activation_rules: { min_days_active: 14, min_revenue_cents: 10000, description: "Vendor must be active 14+ days AND generate $100+ in fulfilled orders." }
        },
        garagebot: {
          purchase_commission: "10% of GarageBot affiliate commission",
          pro_conversion_bonus: "$5 one-time when referred user upgrades to Pro",
          pro_recurring: "$2/month per active Pro referral",
          qualification_threshold: "$100 total referred purchases"
        }
      },
      payout: {
  ...
  app.get("/api/affiliates/lookup/:code", async (req: Request, res: Response) => {
    try {
      const { code } = req.params;

      if (!isValidEcosystemAffiliateId(code)) {
      ...
      if (platform === "tl") {
        const [affiliate] = await db.select().from(ecosystemAffiliates)
          .where(eq(ecosystemAffiliates.trustLayerId, code)).limit(1);
        if (!affiliate) {
          return res.status(404).json({ error: "Affiliate not found on Trust Layer" });
        }
        return res.json({
          source: "trust_layer",
          platform: "tl",
      ...
      if (platform === "garagebot") {
        const localRecord = await db.select().from(ecosystemAffiliates)
          .where(eq(ecosystemAffiliates.trustLayerId, code)).limit(1);

        try {
          const baseUrl = ECOSYSTEM_APP_URLS.garagebot;
          const response = await fetch(`${baseUrl}/api/affiliate-program/trustlayer/${code}`, {
          ...
          if (response.ok) {
            ...
            return res.json({
              ...
              platform: "garagebot",
              affiliateCode: code,
              localRecord: localRecord[0] || null,
              remoteData
            });
          }
          ...
          if (response.status === 404) {
            if (localRecord[0]) {
              return res.json({ source: "trust_layer_local", platform: "garagebot", affiliateCode: code, localRecord: localRecord[0], remoteData: null, note: "Affiliate registered locally but not found on GarageBot" });
            }
            return res.status(404).json({ error: "Affiliate not found on GarageBot or Trust Layer" });
          }
        } catch (fetchErr: any) {
          if (localRecord[0]) {
            return res.json({ source: "trust_layer_local", platform: "garagebot", affiliateCode: code, localRecord: localRecord[0], remoteData: null, note: "GarageBot API unreachable — showing local record only" });
          }
          return res.status(502).json({ error: "GarageBot API unreachable and no local record found" });
        }

        return res.status(502).json({ error: "Failed to reach GarageBot affiliate API" });
      }
      ...
      return res.json({
        ...
        affiliateCode: code,
        data: null,
        note: `Affiliate data for ${platform} platform codes must be queried from the originating platform directly.`
      });
    } catch (error: any) {
      if (error?.name === "TimeoutError" || error?.name === "AbortError") {
        return res.status(504).json({ error: "External platform request timed out" });
      }
      console.error("Affiliate lookup error:", error);
      res.status(500).json({ error: "Failed to look up affiliate" });
    }
  });
  ...
  app.post("/api/affiliates/register", async (req: Request, res: Response) => {
    try {
      ...
      if (!isValidEcosystemAffiliateId(affiliateId)) {
        return res.status(400).json({ error: "Invalid affiliate ID format. Must match ecosystem standard (tldc-/he-/tl-) or GarageBot (GB-XXXXXX) format." });
      }
  ...
  app.get("/api/affiliates/:trustLayerId/dashboard", async (req: Request, res: Response) => {
    try {
      ...
      if (!isValidEcosystemAffiliateId(trustLayerId)) {
...
const APP_URL_MAP: Record<string, string> = {
  "orbitstaffing": "https://orbit.tlid.io",
  "orbit-staffing": "https://orbit.tlid.io",
  "darkwave-staffing": "https://orbit.tlid.io",
  "lotopspro": "https://lotopspro.tlid.io",
  "lotops-pro": "https://lotopspro.tlid.io",
  "orby": "https://orby.tlid.io",
  "garagebot": "https://garagebot.tlid.io",
  "garagebot-prod": "https://garagebot.tlid.io",
  "brew-board": "https://brewboard.tlid.io",
  "brewandboard": "https://brewboard.tlid.io",
  "darkwave-pulse": "https://pulse.tlid.io",
  "paintpros": "https://paintpros.tlid.io",
  "nashpaintpros": "https://nashpaintpros.tlid.io",
  "lumepaint": "https://lumepaint.co",
  "strikeagent": "https://strikeagent.tlid.io",
  "strike-agent": "https://strikeagent.tlid.io",
  "vedasolus": "https://vedasolus.tlid.io",
  "veda-solus": "https://vedasolus.tlid.io",
  "darkwave-studios": "https://darkwavestudios.tlid.io",
  "darkwavestudios": "https://darkwavestudios.tlid.io",
  "tradeworks-ai": "https://tradeworks.tlid.io",
  "tradeworksai": "https://tradeworks.tlid.io",
  "chronicles": "https://chronicles.tlid.io",
  "the-arcade": "https://arcade.tlid.io",
  ...
  "the-void": "https://thevoid.tlid.io",
  "intothevoid": "https://thevoid.tlid.io",
  "verdara": "https://verdara.tlid.io",
  "arbora": "https://arbora.tlid.io",
  "trust-home": "https://trusthome.tlid.io",
  "trust-vault": "https://trustvault.tlid.io",
  "guardian-scanner": "https://guardianscanner.tlid.io",
  "guardian-screener": "https://guardianscreener.tlid.io",
  "trustshield": "https://trustshield.tlid.io",
  "trust-layer": "https://trustlayer.tlid.io",
  "tlid": "https://tlid.tlid.io",
...
function getLocalEcosystemApps(): EcosystemApp[] {
  return [
    ...
    },
    {
      ...
      verified: true,
      users: "DarkWave Verified",
      url: "https://nashpaintpros.tlid.io",
    },
    {
      id: "paintpros",
      name: "PaintPros",
    ...
    },
    {
      ...
      featured: true,
      users: "DarkWave Verified",
      url: "https://darkwavestudios.tlid.io",
    },
    {
      id: "tradeworks-ai",
      name: "TradeWorks AI",
    ...
    },
    {
      ...
      verified: true,
      users: "DarkWave Verified",
      url: "https://driverconnect.tlid.io",
    },
    {
      id: "trust-home",
      name: "Trust Home",
      category: "Core",
      description: "Your personal Trust Layer dashboard. View your membership, Signal balance, blockchain stamps, and ecosystem activity all in one place.",
      hook: "Your Trust Layer Home Base",
      tags: ["Dashboard", "Identity", "Core", "Membership"],
